...
7. **Source-revision identity gate (issue #361 step 1):** three identities — printed `doc_id` (citations/family search), `source_rev` (`vendor|product|version|sha256` normalized; stamped on payloads + inventory for the 361B selector migration), committed generation. Planning dedups byte-identical copies onto the lexicographic winner and aborts fail-closed on cross-revision `doc_id` collisions before any parse/delete/upsert; unreadable files never join the map. Destructive selectors still key on bare `doc_id` until the 361B migration. Details: `docs/ingest.md` §9.
...
- **Representation manifest (issue #362 step 1, record-only):** every non-dry ingest commits the stored-representation contract (extraction rules, identity schema, dense mode/model/operator-revision/dim, contextual recipe, sparse model/weights revision, record-only query prefix) as one fixed-ID point in `<collection>__completions` plus a run-log line; completions and inventory records carry its digest. Enforcement (ingest preflight + serving readiness) is the 362B follow-up.
