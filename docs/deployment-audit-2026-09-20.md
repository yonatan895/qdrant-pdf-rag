...
| Host log overlay stops after the first failed pod | Ingest continued in the retry pod while the original host stream had ended | Launcher now follows successive retry pods and terminates/reaps its owned log child when the Job wait returns; both terminal outcomes passed hermetic tests, and the exact overlay passed a read-only check against the active Job |
