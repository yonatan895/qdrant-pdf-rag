...
INIT_REF = "SA23-1380-09"  # z/OS MVS Initialization and Tuning Reference (parmlib owner)
...
LEG = [
    ("NFS mount error return codes", "diagnostic", ["SC23-6883-00"]),
    ("DFSORT sysin squeeze parameter", "syntax", ["SC23-6878-00"]),
    ("hsm journal filling up troubleshooting", "diagnostic", ["SC23-6871-04"]),
    ("SC23-6883-00", "doc_number", ["SC23-6883-00"]),
    ("SC23-6878-00", "doc_number", ["SC23-6878-00"]),
    ("SC23-6871-04", "doc_number", ["SC23-6871-04"]),
    ("Network File System mount", "diagnostic", ["SC23-6883-00"]),
    ("DFSORT application programming", "diagnostic", ["SC23-6878-00"]),
    ("DFSMShsm storage administration", "diagnostic", ["SC23-6871-04"]),
    ("DFSMShsm implementation customization", "diagnostic", ["SC23-6869-02"]),
    ("DFSORT messages codes diagnosis", "diagnostic", ["SC23-6879-00"]),
    ("DFSORT tuning", "comparative", ["SC23-6882-01"]),
]
...
e("MSG-09", "ARC0500I appeared in DFSMShsm output. What does the message book say it reports and what action follows?", "message_id", ["SA38-0669-05"])
e("MSG-10", "CBR3300I was issued by the tape library. What does the manual say it reports?", "message_id", ["SA38-0671-04"])
e("MSG-11", "ATR218I came from RRS. What does the message document and what should the operator do?", "message_id", ["SA38-0670-03"])
e("MSG-12", "What does AHL124I report during a GTF trace?", "message_id", ["SA38-0668-04"])
e("MSG-13", "IEE400I showed up at IPL. What condition does the message book describe?", "message_id", ["SA38-0674-06"], heading="IEE400I")
e("MSG-14", "RMM issued EDG4004I during a tape mount. What is documented?", "message_id", ["SA38-0672-03"])
e("MSG-15", "IGD002I appeared during allocation. What does the message indicate per the manual?", "message_id", ["SA38-0675-08"])
e("MSG-16", "The console is showing IEE012A. What reply does the manual document?", "message_id", ["SA38-0674-06"], heading="IEE012A")
e("MSG-17", "What does ATB002I report about APPC/MVS scheduling?", "message_id", ["SA38-0670-03"])
e("MSG-18", "IXG312E during log stream processing: what does the message book document?", "message_id", ["SA38-0677-04"])
e("MSG-19", "GRS issued ISG009D. Which replies does the manual document?", "message_id", ["SA38-0676-07"])
e("MSG-20", "EDG0215D came from RMM. What reply options are documented?", "message_id", ["SA38-0672-03"])
e("MSG-21", "What does CRU113I report about the tape library drive?", "message_id", ["SA38-0671-04"])
e("MSG-22", "DADSM produced ADR918I. What is the documented meaning?", "message_id", ["SA38-0668-04"])
e("MSG-23", "ADR497E during volume processing: what does the message book say?", "message_id", ["SA38-0668-04"])
e("MSG-24", "IEE196I appeared with display output. What does the manual say it is?", "message_id", ["SA38-0674-06"], heading="IEE196I")
e("MSG-25", "IOS207I for a device: what condition and operator action are documented?", "message_id", ["SA38-0676-07"], must_not_msgs=["IOS208I"],
     note="sibling trap: IOS208I is a real adjacent ID in the same volume and must not supply the context")
e("MSG-26", "IOS208I was issued for an I/O device. What does the manual document?", "message_id", ["SA38-0676-07"], must_not_msgs=["IOS207I"],
     note="sibling trap: IOS207I is a real adjacent ID in the same volume")
e("MSG-27", "IEF238D is waiting for a reply about a volume. What reply texts are documented?", "message_id", ["SA38-0675-08", "SA38-0666-07"])
e("MSG-28", "ARC0184I from DFHSM: what does the message book say it reports?", "message_id", ["SA38-0669-05", "SC23-6871-04"])
e("MSG-29", "What does ARC0835I report in DFSMShsm processing?", "message_id", ["SA38-0669-05"])
e("MSG-30", "GTF is asking AHL101A for options. Which replies does the manual document?", "message_id", ["SA38-0668-04"])
e("MSG-31", "AMD001A appeared during SMS processing. What does the message document?", "message_id", ["SA38-0668-04"])
e("MSG-32", "ICU010I was issued. What does the message book say about it?", "message_id", ["SA38-0673-06"])
e("MSG-33", "ARC0734I from DFSMShsm: what is the documented meaning?", "message_id", ["SA38-0669-05"])
e("MSG-34", "IGD17002I during SMS allocation: what does it report?", "message_id", ["SA38-0675-08"])
...
# the 'Part 2. Members of SYS1.PARMLIB' part level) -----------
e("SYN-10", "Code a VATLSTxx entry that marks a user volume permanently resident, using only documented VATLSTxx syntax.", "syntax", [INIT_REF], heading="Chapter 83. VATLSTxx (volume attribute list)", syntax_pattern=r"(?i)\bVATLSTxx\b")
e("SYN-11", "Write a SCHEDxx PPT entry that gives a program non-swappable status, per documented SCHEDxx syntax.", "syntax", [INIT_REF], heading="Chapter 79. SCHEDxx (PPT, master trace table, and abend codes for automatic restart)", syntax_pattern=r"(?i)\bSCHEDxx\b")
e("SYN-12", "Define a new MCS console in CONSOLxx with a documented name and auth level.", "syntax", [INIT_REF], heading="Chapter 23. CONSOLxx (console configuration definition)", syntax_pattern=r"(?i)\bCONSOLxx\b")
e("SYN-13", "Write an MPFLSTxx entry that suppresses a noisy message ID on the operator console.", "syntax", [INIT_REF], heading="Chapter 73. MPFLSTxx (message processing facility list)", syntax_pattern=r"(?i)\bMPFLSTxx\b")
e("SYN-14", "Code an LNKLSTxx statement that adds a user library to the LNKLST set after SYS1.LINKLIB.", "syntax", [INIT_REF], heading="Chapter 69. LNKLSTxx (LNKLST concatenation)", syntax_pattern=r"(?i)\bLNKLSTxx\b")
e("SYN-15", "Draft an LPALSTxx entry adding a library to LPA, following the documented syntax.", "syntax", [INIT_REF], heading="Chapter 71. LPALSTxx (LPA library list)", syntax_pattern=r"(?i)\bLPALSTxx\b")
e("SYN-16", "Add a new SVC number with IEASVCxx using only documented parameters.", "syntax", [INIT_REF], heading="Chapter 53. IEASVCxx (installation-defined SVCs)", syntax_pattern=r"(?i)\bIEASVCxx\b")
e("SYN-17", "Define a GRS ring configuration in GRSCNFxx per the documented syntax.", "syntax", [INIT_REF], heading="Chapter 34. GRSCNFxx (global resource serialization configuration)", syntax_pattern=r"(?i)\bGRSCNFxx\b")
e("SYN-18", "Use DIAGxx to enable common storage tracking as documented.", "syntax", [INIT_REF], heading="Chapter 30. DIAGxx (control common storage tracking and GFS trace)", syntax_pattern=r"(?i)\bDIAGxx\b")
e("SYN-19", "Set unit affinity and allocation system defaults in ALLOCxx using documented statements.", "syntax", [INIT_REF], heading="Chapter 4. ALLOCxx (allocation system defaults)", syntax_pattern=r"(?i)\bALLOCxx\b")
e("SYN-20", "Write CLOCKxx statements setting the time zone for UTC with the documented keywords.", "syntax", [INIT_REF], heading="Chapter 16. CLOCKxx (time of day parameters)", syntax_pattern=r"(?i)\bCLOCKxx\b")
e("SYN-21", "Define XCF signalling paths in COUPLExx with documented syntax.", "syntax", [INIT_REF], heading="Chapter 24. COUPLExx (cross-system coupling facility (XCF) parameters)", syntax_pattern=r"(?i)\bCOUPLExx\b")
e("SYN-22", "Tune real storage management via IEAOPTxx with only documented parameters.", "syntax", [INIT_REF], heading="Chapter 50. IEAOPTxx (OPT parameters)", syntax_pattern=r"(?i)\bIEAOPTxx\b")
e("SYN-23", "Write an IEASLPxx SLIP SET statement to capture a dump for an abend in a named module.", "syntax", [INIT_REF], heading="Chapter 52. IEASLPxx (SLIP commands)", syntax_pattern=r"(?i)\bSLIP\b")
e("SYN-24", "Define a subsystem named BATCHA in IEFSSNxx keyword form per the documented syntax.", "syntax", [INIT_REF], heading="Chapter 58. IEFSSNxx (subsystem definitions) - keyword parameter form", syntax_pattern=r"(?i)\bIEFSSNxx\b")
e("SYN-25", "Configure a System Logger structure in IXGCNFxx with documented keywords.", "syntax", [INIT_REF], heading="Chapter 68. IXGCNFxx (system logger initialization parameters)", syntax_pattern=r"(?i)\bIXGCNFxx\b")
e("SYN-26", "Write IDCAMS DEFINE CLUSTER syntax for a KSDS with the documented parameters.", "syntax", ["SC23-6846-02"], syntax_pattern=r"(?i)\bDEFINE\b")
e("SYN-27", "Use documented IDCAMS ALTER syntax to change a data set's share options.", "syntax", ["SC23-6846-02"], syntax_pattern=r"(?i)\bALTER\b")
e("SYN-28", "Code a RACF RDEFINE command for a general resource profile with UACC NONE, per documented syntax.", "syntax", ["SA23-2292-03"], syntax_pattern=r"(?i)\bRDEFINE\b")
e("SYN-29", "Write the documented RACF CONNECT command syntax to add a user to a group.", "syntax", ["SA23-2292-03"], syntax_pattern=r"(?i)\bCONNECT\b")
e("SYN-30", "Write a REXX exec that uses ARG and SAY per the documented TSO/E REXX instructions.", "syntax", ["SA32-0982-00"], syntax_pattern=r"(?i)\bARG\b")
e("SYN-31", "Show documented TSO/E ALLOCATE syntax allocating a new data set with a block size.", "syntax", ["SA32-0975-01"], syntax_pattern=r"(?i)\bALLOCATE\b")
...
e("SYN-34", "Write DFSORT INCLUDE and SORT FIELDS statements for filtering and sorting fixed records, using documented syntax.", "syntax", ["SC23-6878-00"], syntax_pattern=r"(?i)\bSORT\b")
e("SYN-35", "Write JCL with a DD statement concatenating three data sets, following documented DD syntax.", "syntax", ["SA23-1385-03"], syntax_pattern=r"(?m)^\s*//")
e("SYN-36", "Show documented ICKDSF INIT command syntax to initialize a DASD volume.", "syntax", ["GC35-0033-41"], syntax_pattern=r"(?i)\bINIT\b")
...
e("DIA-13", "DFHSM issued ARC1001I during recall. What do the message and its reason code document?", "diagnostic", ["SA38-0669-05", "SC23-6871-04"])
e("DIA-14", "IGW467I from SMB serving: what does the manual say to check?", "diagnostic", ["SA38-0676-07"])
e("DIA-15", "An SVC dump was taken: what is the documented IPCS path to analyze the failing module?", "diagnostic", ["SA23-1384-00", "SA23-1382-03"])
e("DIA-16", "IBM Health Checker reported an exception: what does the check output tell the operator to do?", "diagnostic", ["SC23-6843-05"])
e("DIA-17", "Catalog DEFINE failures are cascading: which book documents catalog diagnosis and what steps does it specify?", "diagnostic", ["SC23-6863-01", "SC23-6861-02"])
e("DIA-18", "A tape mount is stuck pending: what does the tape documentation say to check on the drive and the library?", "diagnostic", ["SC23-6854-00"])
e("DIA-19", "GRS shows enqueue contention between two jobs: what does the documentation recommend to diagnose it with DISPLAY GRS?", "diagnostic", ["SA38-0666-07", INIT_REF])
e("DIA-20", "SLIP was set for an S0C4: how do SLIP and IEASLPxx document the trap and dump flow?", "diagnostic", [INIT_REF])
e("DIA-21", "A Language Environment CEExxxx abend occurred: what is the documented debugging path?", "diagnostic", ["GA32-0908-03"])
e("DIA-22", "SMF ran out of space: what does the SMF documentation say about managing SYS1.MAN data sets and switching?", "diagnostic", ["SA38-0667-09"])
e("DIA-23", "XCF signalling between two systems failed: what diagnosis does the sysplex documentation specify?", "diagnostic", ["SA23-1399-03", "SA38-0658-02"])
e("DIA-24", "IEA995I says a dump was produced: what documented next steps lead to the dump data set?", "diagnostic", ["SA38-0673-06", "SA23-1374-02"])
e("DIA-25", "A batch step is looping on CPU: what do the RMF and SMF books document to confirm the address space and its consumption?", "diagnostic", ["SC34-2665-03", "SA38-0667-09"])
e("DIA-26", "ICH408I denied a data set access: which RACF documentation explains how to read the profile and access fields?", "diagnostic", ["SA23-2287-03"])
e("DIA-27", "A DFSMShsm-managed volume is at capacity: what migration actions does the storage administration book document?", "diagnostic", ["SC23-6871-04"])
e("DIA-28", "A REXX exec failed with an IRX message: what do the IRX explanations say to check?", "diagnostic", ["SA32-0982-00"], heading="IRX")
e("DIA-29", "A user cannot enter TSO commands after logon: what does the TSO/E documentation say about command availability and IKJTSOxx?", "diagnostic", ["SA32-0975-01", INIT_REF])
e("DIA-30", "Print output is stuck in the JES spool: what do the SDSF and JES2 books document for holding and releasing output?", "diagnostic", ["SA23-2274-09", "SA32-0991-02"])
e("DIA-31", "A data space hit a storage constraint: what does the Extended Addressability book document for diagnosis?", "diagnostic", ["SA23-1394-01"])
e("DIA-32", "ICKDSF reported an error during volume initialization: where are the ICK message explanations and recovery?", "diagnostic", ["GC35-0033-41"])

# --- new: comparative / tuning --------------------------------------------------

e("CMP-07", "Compare documented DFSMShsm migration level 1 versus level 2: when is each used?", "comparative", ["SC23-6871-04"])
e("CMP-08", "Compare IDCAMS DEFINE versus ALTER for changing SMS-managed data set attributes, per the AMS command book.", "comparative", ["SC23-6846-02"])
e("CMP-09", "Compare IEASYSxx versus the SET command for changing system parameters: which is documented as IPL-only?", "comparative", [INIT_REF, "SA38-0666-07"])
e("CMP-10", "Compare RACF PERMIT versus CONNECT: which applies to data set profiles and which to user-group membership?", "comparative", ["SA23-2292-03"])
e("CMP-11", "Compare WLM service classes versus report classes when tuning batch goals.", "comparative", ["SC34-2662-05"])
e("CMP-12", "Compare SMFPRMxx SYS versus SUBSYS recording options for reducing SMF volume.", "comparative", ["SA38-0667-09", INIT_REF])
e("CMP-13", "Compare SLIP TRACE versus GTF for capturing diagnostic data: what does the documentation recommend for each?", "comparative", [INIT_REF, "SA23-1378-01"])
e("CMP-14", "Compare binder REPLACE versus ALIAS options in program management.", "comparative", ["SA23-1393-01"])
e("CMP-15", "Compare TSO/E TRANSMIT and RECEIVE for data set exchange per the command reference.", "comparative", ["SA32-0975-01"])
e("CMP-16", "Compare ICKDSF INIT versus REVAL for DASD volume preparation.", "comparative", ["GC35-0033-41"])
...
e("VER-09", "Encryption Facility for z/OS OpenPGP usage: which book covers it and what version does its title carry?", "version", ["SA23-2230-07"],
     note="sibling-book pair with VER-10 (SA23-2229-03 is the planning book of the same product, not the OpenPGP user guide); no must_not by design, see VER-10 note")
e("VER-10", "Encryption Facility planning and customizing: which book covers key configuration and what edition suffix does it carry?", "version", ["SA23-2229-03"],
     # No must_not on the sibling by design (2026-09-06 re-bind): the sibling
     # user guide shares the product vocabulary (measured: correct book #2,
     # sibling parameter-index #3), so top-5 exclusion exceeds retrieval
     # resolution while the precision signal that matters — correct book
     # outranks sibling — holds and is measured by rank. Sibling
     # discrimination from co-retrieved excerpts is the answer tier's job.
     note="sibling-book pair with VER-09; edition labeling is the asserted behavior")
...
e("VER-12", "JES2 initialization statements: state which z/OS edition the excerpts cover before quoting IAZ/HASJ syntax.", "version", ["SA32-0992-02"],
     note="single-edition corpus: the answer must name the covered edition rather than merging releases")
e("VER-13", "Infoprint Server Print Transforms: which product version do the excerpts document and how is it labeled?", "version", ["aokfa102"],
     note="IBM Print Transforms from AFP for Infoprint Server for z/OS (aokfa102); the answer must label the version from the excerpts")
...
e("DIA-33", "A CICS TS 3.1 transaction abended and DFHAC2006 is in the message log. What does the message tell the operator to collect before calling support?", "diagnostic",
     ...
     # expectation) because synthetic-venue PDFs have no page labels, so a
     # page expectation on a dev entry is unverifiable by construction.
     note="DFHAC2006 guidance is on p.66 of the printed CICS messages book")
...
SEED_OVERRIDES = {
    ...
    "MSG-02": ["SA38-0675-08"],          # IEF450I -> Vol 8 (IEF-IGD)
    "MSG-03": ["SA38-0673-06", "SC23-6844-02"],  # IEA794I -> Vol 6 (GOS-IEA) + Problem Management
    "MSG-04": ["SA38-0676-07", "SA23-1373-04", "SA23-2284-02"],  # IRA200E
    "MSG-05": ["SA23-2291-03"],          # ICH408I -> RACF Messages and Codes
    "DIA-01": ["SA23-1379-02", "SA38-0676-07"],  # IRA100E CSA/SQA -> Init&Tuning Guide + Vol 9
    "DIA-03": ["SA38-0674-06", "SC23-6854-00"],  # IOS000I
    "DIA-05": ["SA38-0674-06", "SC23-6885-01"],  # IEC070I
    "DIA-06": ["SA38-0673-06", "SC23-6844-02", "SA23-1374-02"],  # IEA611I
    "CMP-01": ["SA32-0992-02", "SA32-0991-02"],  # JES2 side answerable; no JES3 book (note covers)
    "CMP-02": ["SA38-0667-09", INIT_REF],        # SMFPRMxx
    "CMP-03": [INIT_REF, "SA23-1379-02"],        # CSA/ECSA/SQA
    "CMP-05": ["SC34-2662-05", "SC34-2663-04"],  # WLM (single 2.2 edition; the 2.5 edition left with the old corpus)
    "CMP-06": ["SA23-2287-03", "SA23-2288-03"],  # RACF SETROPTS
    "VER-01": [INIT_REF],                          # LFAREA (single 2.2 edition; the 2.5 edition left with the old corpus)
    "VER-05": ["SA23-2287-03"],                   # RACF version rules: single edition
    "VER-06": [INIT_REF, "GA32-0884-02"],  # BPXPRMxx (single I&T edition; the 2.5 edition left with the old corpus)
    "SYN-01": [INIT_REF],                         # LFAREA -> IEASYSxx chapter
    "SYN-02": [INIT_REF, "SA23-1390-01"],         # COMMNDxx
    "SYN-03": ["SA23-1385-03"],                   # JCL
    # 2026-09-06: OPS/MVS book absent from the V2R2 corpus; SYN-04 abstains
    # via ABSENT_DOMAIN_FALLBACK (no override while the book is unloaded).
    "SYN-05": [INIT_REF],                         # PROGxx APF
    "SYN-08": ["SA23-2292-03"],                   # RACF PERMIT
...
# 2026-09-06 re-bind (V2R2 real corpus): the BatchPipes OS/39 book (SA22-7456-01) and the SCRT book (SC23-6845-22) are no longer in the corpus. The queries stay (they are realistic operator asks and the answer must abstain per corpus absence); flip deliberately if the books return.
# SYN-32/SYN-33 (OPS/MVS AOF rules): the OPS/MVS 14.0 book left with the old corpus. MSG-24: IEE196I is not carried by any loaded book.
E_ABSTAIN = {"VER-07", "VER-08", "VER-11", "SYN-32", "SYN-33", "MSG-24"}
...
def main() -> int:
    ...
    for line in SEED.read_text(encoding="utf-8").splitlines():
        ...
        if mapped["id"] in FORCE_ABSTAIN:
            ...
            if mapped["id"] in ABSENT_DOMAIN_FALLBACK:
                ...
                mapped["query_class"] = "negative"
                mapped["expected_behavior"] = "abstain"
                # an abstain entry produces no answer, so it carries no
                # answer-shape gold (the builder assertion enforces this)
                mapped.pop("syntax_pattern", None)
                if ABSENT_CORRECTIONS.get(mapped["id"]):
                    mapped["note"] = f"{mapped['note']}; {ABSENT_CORRECTIONS[mapped['id']]}"
            else:
                seed_errors.append(
    ...
    for row in E:
        entry = dict(row)
        if entry["id"] in E_ABSTAIN:
            # Corpus-absence abstention (see E_ABSTAIN): the referenced book
            # is not loaded, so the correct behavior is to abstain. Strip
            # the doc expectations here so the abstain schema invariant
            # holds regardless of what the table row still lists.
            entry["expected_doc_ids"] = []
            entry["expected_heading"] = None
            entry["expected_page"] = None
            entry["expected_behavior"] = "abstain"
            # an abstain entry produces no answer: no answer-shape gold, and
            # a note recording the flip for the verify-golden hygiene check
            entry.pop("syntax_pattern", None)
            entry["note"] = (entry.get("note") or "") + (
                "; 2026-09-06 re-bind: referenced book absent from the loaded "
                "corpus, correct outcome is abstention"
            )
        else:
