...
# script mints random sk-local-... keys and prints them once (never committed,
# never written by the script — terminal only). Minted keys live in the
# throwaway Postgres volume: env-passed values survive restarts; unset values
# rotate at each start and replace the prior key. GATEWAY_RESET_KEYS=1 wipes
# the store. Key charset is sk- + alphanumerics/dash: anything else dies
# fail-closed (values render into YAML unquoted).
...
==> Gateway ready: ${BASE} (container ${GATEWAY_NAME}, log /tmp/${GATEWAY_NAME}.log)
    ...
    values below are terminal-only, never written to git.
    Point every leg at the gateway, then run scripts/probe_gateway.py to
    confirm and to get the RERANK_ENDPOINT_ORDER recommendation:
