...
- Trigger split (GitHub only): markdown-only changes run `.github/workflows/markdown.yml` (markdownlint-cli2, config in `.markdownlint-cli2.yaml`) and skip hygiene/pytest; vendored `.agents/` and `vendor/` markdown runs nothing. GitLab keeps hygiene + pytest on every MR.
