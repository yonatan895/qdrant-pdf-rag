...
- Retrieval-accuracy rule: retrieval changes (embedder, chunking, RRF constants, filters, query shape) must show their recall/MRR delta on the golden set (`make eval`, `evals/golden.jsonl`) in the PR body — accuracy changes are never merged unmeasured. Golden entries are doc-level and never pin top-1 across potentially equal-text chunks. The sim tier runs the eval on the synthetic corpus as a regression gate (`test_eval_retrieval_on_synthetic_corpus`).
