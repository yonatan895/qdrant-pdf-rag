...
- Third-party pins live in `images.txt` (Qdrant unprivileged, Jaeger, UBI, oauth-proxy — the last is `sha256:PENDING` until a connected host records it; `pack.sh` skips it and `AGENT_ROUTE=true` deploy fails closed while PENDING). A `requirements.lock.txt` bump requires `make wheelhouse bm25-weights` on CPython 3.14 and a connected image rebuild. Dedicated PR, not drive-by. `qdrant-client` pin tracks the 1.19 server/chart.
...
- Release transfer gate: published-main bundles must pass the manual Windows CRC procedure in `docs/crc-release-verification.md` before production transfer; missing/failed checks block promotion. Transfer the identical tested bundle. Kind remains the fast development rehearsal. CRC overrides change sizing/coordinates only; prove `restricted-v2` with project-assigned IDs, fix admission/storage failures in the owning production configuration, and never grant `anyuid`. Automate only after the manual procedure passes.
