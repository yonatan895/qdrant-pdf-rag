...
if [ "${AIRGAP_DRYRUN:-0}" = "1" ]; then
    ...
    # First-party chart rehearsal (issue #448 H2a, dry-run only): map the
    # resolved operator environment to release values and render the chart.
    # The Kustomize renders above remain the migration oracle; the chart
    # render gates dry-run. Production (non-dry-run) paths below are
    # untouched. --without-ingest-job keeps the steady-state release free
    # of the one-shot Job (D3). The export passes file-sourced values
    # (collection-policy preset, airgap.env) that plain sourcing leaves
    ...
    python3 scripts/airgap/map_values.py --out dist/mainframe-rag-release-values.yaml --without-ingest-job
    helm lint charts/mainframe-rag -f dist/mainframe-rag-release-values.yaml
    helm template app charts/mainframe-rag -f dist/mainframe-rag-release-values.yaml --namespace "$NAMESPACE" > dist/agent-chart-rendered.yaml
    fail_on_placeholders dist/agent-chart-rendered.yaml "agent chart"
    echo "[dryrun] chart values kept at dist/mainframe-rag-release-values.yaml"
    echo "[dryrun] chart manifest kept at dist/agent-chart-rendered.yaml"
else
